...
rm(path.join(config.pack.assetsRoot, config.pack.assetsSubDirectory), err => {
  ...
  webpack(webpackConfig, (err, stats) => {
    ...
    console.log('Copying Python server to dist folder ... \n')
    fs.createReadStream(config.pack.serverFilePath).pipe(fs.createWriteStream(config.pack.targetServerFilePath));
